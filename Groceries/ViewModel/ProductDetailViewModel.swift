    ...
    func serviceCallDetail(productId: Int) {
        ...
        ServiceCall.get(path: path) { responseObj in
            if let response = responseObj as? NSDictionary {
                // Backend trả về ProductResponseDTO trực tiếp, không có "status" và "payLoad"
                self.pObj = ProductModel(dict: response)
            } else {
                self.errorMessage = "Failed to parse product details"
                self.showError = true
            }
        } failure: { error in
